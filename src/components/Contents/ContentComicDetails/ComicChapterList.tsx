...
export const ComicChapterList: React.FC<ComicChapterListProps> = ({
  comicId,
  chapters,
  loading,
}) => {
  return (
    <>
      <Space size={"small"} direction="horizontal">
        <Link
          to={getSingleChapterPath(comicId, chapters[chapters?.length - 1]?.id)}
        >
          <Button danger ghost>
            Đọc từ đầu
          </Button>
            ...
            <List
              ...
              renderItem={(chapter) => (
                <Link
                  className="link"
                  key={chapter.id}
                  to={getSingleChapterPath(comicId, chapter.id)}
                >
                  <List.Item>
